...
if args.debug is True:
    print(os.path.dirname(os.path.realpath(__file__)))
if scriptmode == False:
    if hashlib.sha256(open(path + '/cf.py', "rb").read()).hexdigest() != requests.get("https://raw.githubusercontent.com/chrischan514/Cloudflare-API/main/sha256",headers={'Cache-Control': 'no-cache'}).text:
